...
class Node:
    ...
    def draw_node(self, world):
        ...
        py.draw.circle(world.win, colorScheme[1], (self.x,self.y), NODE_RADIUS-0) # node border

        #draw labels
        if self.type != MIDDLE:
            text = NODE_FONT.render(self.label, 100, TEXT) # Neuron labels
            world.win.blit(text, (self.x + (self.type-1) * ((text.get_width() if not self.type else 0) + NODE_RADIUS + 1), self.y - text.get_height()/2))
...
class Connection:
    ...
    def drawConnection(self, world):
        color = GREEN if self.wt >= 0 else ORANGE
