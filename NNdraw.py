...
class NN:
    ...
    def __init__(self, config, genome, pos):
        ...
        for i,out in enumerate(config.genome_config.output_keys):
            n = Node(out+INPUT_NEURONS, pos[0] + 2*(LAYER_SPACING+2*NODE_RADIUS), pos[1]+int(-h/2 + i*(NODE_RADIUS*2 + NODE_SPACING)), OUTPUT, [ORANGE_PALE, ORANGE, DARK_ORANGE_PALE, SELECTED_NEURON], output_names[i], i)
            self.nodes.append(n)
