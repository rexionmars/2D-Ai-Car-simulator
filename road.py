    ...
    def draw(self, world):
        ...
        if(ROAD_DBG):
            for p in self.ctrl_points:     #EEEEEEEEEEEEEEEEE
                py.draw.circle(world.win, BLUE, (int(p.x), int(p.y)), 4)
            for i in range(len(self.pointsLeft)):
                ...
                py.draw.lines(world.win, BLACK, False, [(self.pointsLeft[i].x, self.pointsLeft[i].y), (self.pointsRight[i].x, self.pointsRight[i].y)], 1)
        else:
            #draw borders
            for i in range(len(self.pointsLeft)):
                ...
                if p.y >= f.y:
                    py.draw.line(world.win, RED, world.getScreenCoords(p.x, p.y), world.getScreenCoords(f.x, f.y), 2)
