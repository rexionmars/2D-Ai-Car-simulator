        ...
        if CAR_DBG:
            for k,s in enumerate(sensors):
                ...
                if s < SENSOR_DISTANCE:
                    py.draw.circle(world.win, ORANGE, world.getScreenCoords(self.x+dx, self.y+dy), 6)
...
def getSensorEquations(self, world):       #restituisce le equazioni delle rette (in variabile y) della macchina in ordine [verticale, diagonale crescente, orizzontale, diagonale decrescente]
    ...
    for i in range(4):
        ...
        if CAR_DBG:             #disegna linee dei sensori
            py.draw.lines(world.win, LINE_SENSOR_COLOR, False, [world.getScreenCoords(self.x+dx, self.y+dy), world.getScreenCoords(self.x-dx, self.y-dy)], 2)
